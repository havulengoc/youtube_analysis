...
```{r}
first_date <- head(watched_video_df$time, 1)
last_date <- tail(watched_video_df$time, 1)
as.period(interval(last_date, first_date))
```

-   First-watched video: April 13, 2021
-   Last-watched video: September 28, 2022
-   Number of days spent on Youtube: 532 days, which is equal to 1 year, 5 months, 14 days, and 18 hours.
-   Total videos I watched during that time: 5409

## 2. A pattern of our watch history
...
Plot of video counts over the year
...
```
It is interesting to look at **how we watched Youtube over the years**. Unfortunately, Ha has the habit of deleting her Youtube history, so the earliest time recorded in her history is April 2021. For Tam, a Youtube-lover, her Youtube history dated back to 2018. There’s a huge gap between these data, but observing the trend over the year brings back interesting memories and milestones of our lives.
...
Based on the plot, we can see that the discrepancy in the number of videos Tam and Ha watched is enormous. Tam watched the most videos in June, with a record of approximately 4100 videos. However, Tam's number of videos decreased gradually when the school year approached. On the contrary, Ha watched significantly less videos, with the lowest recorded in May with an approximate of 13 videos. When she looked back, she was dealing with final exams and traveling back then, so the time dedicated for watching Youtube was little.

```{r}
num_vid_per_month_2022 <- watch_df_union_2022 %>%
  ggplot(aes(x=as.factor(month.abb[month_watch]), fill=owner))+
...
The difference in the categories me and Ha watched:

First is the function to get the percentage of categories in our total watch history from 2022:
...
Next, I generate the plot by running `group_modify`.
```{r}
...
Looking at the category-wise graph, Ha seems to be most interested in Music, with 56.65% of her watched videos belong to this category. However, Tam finds more fun spending time watching People & Blogs videos, with 51.43% of her videos belonging to this category. 

At a glance, our interests do not overlap at all. But when filtering out the most five popular categories, we all have the same ones!

We also would like to take a look at whether our top-watched video categories match with the trending categories in US Youtube. 

```
US_trend <- US_youtube_trending_data %>%
  select(video_id, trending_date, categoryId)

# MERGE CATEGORY NAME
US_trend_df <- US_trend %>%
  filter(lubridate::year(trending_date) == 2022) %>%
  mutate(categoryId = as.character(categoryId)) %>%
  left_join(category_list, by = c("categoryId" = "id"))
glimpse(US_trend_df)

# GRAPH 1: Bar charts on the popularity of category
US_trend_df_plot <- US_trend_df %>%
  group_by(category_name) %>%
  summarise(count = n())  %>%
  arrange(desc(count))

US_trend_df_plot %>%
  ggplot(aes(category_name, count)) +
  geom_col(color = "coral1", fill = "#c1121f") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  labs(x = "Categories", y = "Videos") +
  ggtitle("Distribution of categories among Youtube trending videos")
```

The five most trending categories in the US are Gaming, Entertainment, Music, People & Blogs, and Sports. We also enjoy watching videos in Entertainment, Music, and People & Blogs, but have little interest in Gaming and Sports. 
...
Next step is iterating it over Ha's and my watch history:
```{r}
...
```
Based on our history plot, Ha usually watched Youtube around 3 - 4pm while Tam usually watches around 12 pm to 3pm and 2am to 3am in the morning. 
